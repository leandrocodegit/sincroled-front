...
export class ComandoSincronismoService {
  private readonly baseUrl = `${environment.urlApi}/comando/sincronismo`;
  ...
  /**
   * Envia comando ao IoT via WebFlux (SSE).
   *
   * Ciclo de vida do stream:
   *   ENVIANDO → (OK: device recebeu, stream continua) → SUCESSO | WARN | ERROR | TIMEOUT
   *
   * Payload JSON esperado: { tipoResposta, mensagem, deviceId }
   * Fallback texto: "Falha, não houve resposta" → TIMEOUT
   */
  enviarComando(deviceId: string, tipoConfiguracao?: Tipoconfiguracao): Observable<SincronismoEvento> {
    const url = `${this.baseUrl}/${deviceId}?tipoConfiguracao=${tipoConfiguracao ?? Tipoconfiguracao.LED}&noLoad=true`;


    return new Observable<SincronismoEvento>((observer) => {
      const eventSource = new EventSource(url + `?access_token=${encodeURIComponent(this.ouathService.getAccessToken())}`);
        ...
        this.ngZone.run(() => {
          ...
          try {
            ...
            const evento: SincronismoEvento = {
              ...
              timestamp: new Date(),
              payload: body,
              toast: {severity: body.tipoResposta == 'OK' ? 'info' :'success', detail: body.mensagem}
            };
            ...
            if (isFalha(raw)) {
              ...
              observer.next({
                ...
                deviceId,
                timestamp: new Date(),
                toast: {severity: 'success', detail: raw}
              });
              eventSource.close();
              observer.complete();
            }
